...
func (l *leaderImpl) sendHeartBeats() {
	l.mutex.RLock()
	defer l.mutex.RUnlock()
	fmt.Println(l.addr[17:], "- heartbeats, term", l.term)
	req := appendEntriesRequest{Leader: l.addr, Term: l.term, Entries: []LogEntry{}, PrevIndex: (*l.log).Length() - 1, PrevTerm: (*l.log).LastTerm(), Applied: (*l.log).Applied()}
	...
	for _, url := range l.others {
		go func(url string, data []byte) {
			...
			// if err != nil && resp.StatusCode == http.StatusNonAuthoritativeInfo {
			// 	l.Stop()
			// }
		}(url, data)
	}
	...
	go func() {
		for {
			...
			case info := <-l.sync:
				...
				for _, url := range l.others {
					go func(url string) {
						// fmt.Println("Begin propagating to", url)
...
func (l *leaderImpl) sendUpdateLog(url string, since int) (bool, bool) {
	...
	req := appendEntriesRequest{Leader: l.addr, Term: l.term, Entries: entries, PrevIndex: since - 1, PrevTerm: (*l.log).TermAt(since - 1)}
	data, err := json.Marshal(req)
	if err != nil {
		panic(err.Error())
	}
